...
class SchedulingAgent:
    ...
    def _is_doctor_unavailable(self, db: Session, doctor_id: int, day: date) -> bool:
        return (
            ...
            .filter(DoctorUnavailable.doctor_id == doctor_id, DoctorUnavailable.date == day)
            .count()
            > 0
        )
    ...
    def get_available_slots(self, db: Session, doctor_id: int, days_ahead: int = 5) -> List[datetime]:
